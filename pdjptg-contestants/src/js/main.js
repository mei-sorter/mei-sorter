...
let sorterSrc = "pdjptg-contestants";
...
function start() {
  ...
  const dataFilter = parseInt(checkedFilter.value);

  document.getElementById('filter').classList.add('hidden')

  /** Copy data into sorting array to filter. */
  characterDataToSort = characterData.filter(data => {
    return data.rank <= dataFilter;
  });
...
function display() {
  ...
  const charNameDisp = (name, displayName) => {
    return `<p title="${name}">${displayName}</p>`;
  };
  ...
  const leftCharName = leftChar.last_name ? `${leftChar.last_name}<br>${leftChar.first_name}` : leftChar.name;
  const rightCharName = rightChar.last_name ? `${rightChar.last_name}<br>${rightChar.first_name}` : rightChar.name;
  

  document.querySelector('.left.sort.text').innerHTML = charNameDisp(leftChar.name, leftCharName);
  document.querySelector('.right.sort.text').innerHTML = charNameDisp(rightChar.name, rightCharName);
...
function result(imageNum = 3) {
  ...
  const imgRes = (char, num) => {
    const charName = char.last_name ? `${char.last_name}<br>${char.first_name}` : char.name;
    
    const charTooltip = char.name !== charName ? char.name : '';

    return `
    <div class="result-card" title=${charTooltip}>
      <img src="${char.img}" alt="" srcset="" class="result-picture">
      <div class="result-rank">${num}</div>
      <div class="result-name">${charName}</div>
    </div>`
  }
  const textRes = (char, num) => {
    const charNames = char.name.split(' ');
    const charName =`${charNames[0]}<br>${charNames[1]}`;
  ...
  const resultTableImg = document.querySelector('.results.img');
  const resultTableText = document.querySelector('.results.text');
  ...
  characterDataToSort.forEach((val, idx) => {
    ...
    if (imageDisplay-- > 0) {
      resultTableImg.insertAdjacentHTML('beforeend', imgRes(character, rankNum));
    } else {
      resultTableText.insertAdjacentHTML('beforeend', textRes(character, rankNum));
    }
